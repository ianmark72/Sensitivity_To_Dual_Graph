...
def gerrychain_score(proposal_graph, graph, config, updaters, epsilon, ideal_population, gerrychain_steps, accept, k):
    ...
    if config['optimization_for'] == "R":
        seat_score  = statistics.mean(seats_won_for_republicans[config["GERRYCHAIN_SCORE_BURN_IN"]:])
    if config['optimization_for'] == "D":
        seat_score  = statistics.mean(seats_won_for_democrats[config["GERRYCHAIN_SCORE_BURN_IN"]:])
    return seat_score


def test_score(proposal_graph, graph, config, updaters, epsilon, ideal_population, gerrychain_steps, accept, k):
    print("num edges of proposal graph", len(proposal_graph.edges()))
    print("num edges of current graph", len(graph.edges()))
...
def ising_score(proposal_graph, graph, config, updaters, epsilon, ideal_population, gerrychain_steps, accept, k):
    ...
    return np.log(displacement_energy)
