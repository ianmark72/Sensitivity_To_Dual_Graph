...
def main():
    ...
    config = {
        'VTD_shp_file' : "./data/NC_VTD.shp",
        'sub_partition_shape_file' : "./data/tl_2016_37_tabblock10/tl_2016_37_tabblock10.shp",
...
def impute_down(coarse_partition = "VTD", coarse_id = "vtdid", coarse_field_to_impute = "VOTE",
    ...
    joined_with_geos  = fine_with_map.merge(coarse_partition,
    ...
    joined_with_geos['geo_intersect'] = fine_geos.intersection(gpd.GeoSeries(joined_with_geos.geometry_coarse).to_crs("ESRI:103500"))
    joined_with_geos['geo_fine_area'] = fine_geos.area
    joined_with_geos['intersect_area'] = gpd.GeoSeries(joined_with_geos.geo_intersect).to_crs("ESRI:103500").area
...
def partition_blocks_by_vtds(VTD_shp_file, 
    ...
    filename = "'./processed_data_outputs/blocks_with_votes.shp'"
    ...
    block_id = sub_partition_block_id
    ...
    fields = ['VTD', sub_partition_block_id, 'geometry', 'area_proportion' ] +  vote_fields
    geos_simple = geo_with_vtd_shape_data.rename(columns = {"geometry_block" : "geometry" })[fields]
    geo_with_pop = geos_simple.merge(pops, left_on = sub_partition_block_id, right_on = sub_partition_block_id_pop_table,)
    ...
    gdf_subset = gpd.GeoDataFrame(gdf_subset, geometry="geometry", crs= "ESRI:103500")#.head(10)
...
def generate_block_level_graph(blocks = None):
    ...
    if blocks is None:
        blocks = gpd.read_file("./processed_data_outputs/blocks_with_votes.shp").to_crs("EPSG:4269")
    
    # Generate graph

    blocks['pos'] = blocks.centroid
    blocks = blocks.rename(columns = { "EL16G_GV_D_block" : "dem", "EL16G_GV_R_block" : "rep"})
    blocks['dem_prop'] = blocks.dem / blocks.TOTPOP_block
    blocks['rep_prop'] = blocks.rep / blocks.TOTPOP_block
